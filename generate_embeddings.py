...
class MetricTrainer:
    ...
    def predict(self, items, weights):
        ...
        with torch.inference_mode():
            for batch in loader:
                result.append(self.model(batch[0], batch[1]).detach())
...
if __name__ == '__main__':
    ...
    data_agg = data[['user_id', item_column, 'request_cnt']].groupby(['user_id', item_column]).aggregate([('request_cnt', 'sum')]).reset_index()
    item_set = set(data_agg[item_column])
    ...
    items = np.array(data_agg[item_column].map(item_dict))
